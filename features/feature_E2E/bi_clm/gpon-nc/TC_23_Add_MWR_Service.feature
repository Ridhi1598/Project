@createMWRService @TC23 @pon @mwr
Feature: TC_23 Sequence: BI_CLM Add MWR Service
  This feature tests the end to end functionality of service creation and adding mwr

  Scenario Outline: TC_23 - <action> Service - Provider Managed ipv4 BGP Default Routes
    ...
    And I create "sftp" connection with "primary" "device" and download "before" "config" for "<action>" scenario
    And I create "sftp" connection with "secondary" "device" and download "before" "config" for "<action>" scenario
    ...
    And I create "sftp" connection with "primary" "device" and download "after" "config" for "<action>" scenario
    And I create "sftp" connection with "secondary" "device" and download "after" "config" for "<action>" scenario
    When I find the diff between the "before" and "after" config for "mwr" for "<action>" service
    And I find the diff between the "before" and "after" config for "primary" for "<action>" service
    And I find the diff between the "before" and "after" config for "secondary" for "<action>" service
    Then I validate that the "config diff" is successfully matched with "expected config diff" for "mwr" for "<action>" service
    And I validate that the "config diff" is successfully matched with "expected config diff" for "primary" for "<action>" service
    And I validate that the "config diff" is successfully matched with "expected config diff" for "secondary" for "<action>" service
