@juniperGpon @juniper @gpon @tc141 @rollback
Feature: TC_141 sequence: Create, Update and Delete BI Service
  ...
  @createUpdateDeleteBIService @TC141
  ...
  Scenario: Create a service which fails
    Given I read test data for "179"
    And I generate access token for authorization
    When I set BI "REST" url
    When I Set "requestType" api endpoint for testcase "179"
    When I Set HEADER param request "Content-Type" as "application/json" for BI
    And I Set HEADER param request "Authorization" as "access-token" for BI
    And Set request Body for BI
    And Send HTTP request for BI
  ...
  @createUpdateBIService @TC141 @deleteService
