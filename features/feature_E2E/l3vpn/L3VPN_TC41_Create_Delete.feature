@OA2501-1
Feature: TC_41: Create and Delete GRF QPA test
  ...
  Scenario: Create/Update service with valid payload and in valid state
    ...
    When I run "show running-config policy-map WAN_IN_Global" command and store the device config
    Then I validate the output of "policy-map_WAN_IN_Global" section at "default-level"
    ...
    When I run "show running-config policy-map LAN_IN_Global" command and store the device config
    Then I validate the output of "policy-map_LAN_IN_Global" section at "default-level"
    ...
    When I run "show running-config policy-map LAN_OUT_GLOBAL" command and store the device config
    Then I validate the output of "policy-map_LAN_OUT_GLOBAL" section at "default-level"
    ...
    When I run "show running-config class-map WAN_Global" command and store the device config
    Then I validate the output of "class-map_match-any_WAN_Global" section at "default-level"
    ...
    When I run "show running-config class-map LAN_Global" command and store the device config
    Then I validate the output of "class-map_match-any_LAN_Global" section at "default-level"
  ...
  @deleteService @TC_101
  Scenario: Delete the created service for TC_101 which is in valid state
    When I set L3VPN "REST" url
    Then I set api endpoint for testcase "101" for l3vpn
