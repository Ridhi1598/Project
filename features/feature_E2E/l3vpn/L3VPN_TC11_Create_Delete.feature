@ISR_4221_2
Feature: TC_11: Create and Delete GRF QPV Test
  ...
  Scenario: Create/Update service with valid payload and in valid state
    ...
    Then I validate the output of "policy-map_WAN_OUT_Global_class WAN_MANAGEMENT" section
    Then I validate the output of "policy-map_WAN_OUT_Global_class WAN_ROUTING" section
    Then I validate the output of "policy-map_WAN_OUT_Global_class QOS_GROUP_AF1" section
    Then I validate the output of "policy-map_WAN_OUT_Global_class QOS_GROUP_AF2" section
    Then I validate the output of "policy-map_WAN_OUT_Global_class QOS_GROUP_AF3" section
    Then I validate the output of "policy-map_WAN_OUT_Global_class QOS_GROUP_EF" section
  ...
  @deleteService @TC_71
  Scenario: Delete the created service for TC_71 which is in valid state
    When I set L3VPN "REST" url
    Then I set api endpoint for testcase "71" for l3vpn
    And I Set HEADER param request "Content-Type" as "application/json" for L3VPN
    And I Set HEADER param request "Authorization" as "access-token" for L3VPN
    And Set request Body for L3VPN
    When Send Http request for L3VPN
