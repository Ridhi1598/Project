Feature: Negative - Add empty BNG name
  This feature tests the negative scenario of BNG node name with minimum length

  Scenario: Negative - Add BNG node name with minimum length
    ...
    Then I fill the required parameter for "minimum_length" of BNG name
    Then I click on "firstBNGAdd" button
    Then I validate the error message for "minimum_length_bng" test
