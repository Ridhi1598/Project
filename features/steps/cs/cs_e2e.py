...
@step('I "{action}" that the request "{status}" is "{requestStatus}" for cs')
...
@step('I prepare "{source}" connection information for "{type}" for cs')
def device_info_cs(context, source, type):
    global deviceInfo
    ...
    deviceInfo["host"] = GlobalVar.testParams.get(f'{type}Ip')


@step('I create "{connType}" connection with "{source}" for cs')
def create_conn_cs(context, connType, source):
...
@step('I close "{connType}" connection with "{source}" for cs')
def close_conn_cs(context, connType, source):
    DeviceConnect.stop(context, GlobalVar.connection, connType)


@step('I download device "{config}" for "{scenario}" scenario for "{type}" for "{action}" service for cs')
def fetch_config_cs(context, config, scenario, type, action):
    GlobalVar.testParams[GlobalVar.test_case][f'{scenario}_{config}_{type}_{action}'] = \
        DeviceConnect.fetch_config(context, GlobalVar.connection, scenario, GlobalVar.test_case, type,
                                   GlobalVar.testParams.get(type), action)


@step(
    'I create "{connType}" connection with "{deviceType}" "{source}" and download "{scenario}" "{config}" for "{action}" cs scenario')
def device_config(context, connType, deviceType, source, scenario, config, action):
    device_info_cs(context, source, deviceType)
    create_conn_cs(context, connType, source)
    fetch_config_cs(context, config, scenario, deviceType, action)
    # close_conn_cs(context, connType, source)


@step('I find the diff between the "{before}" and "{after}" config for "{type}" for "{action}" cs service')
...
@step(
    'I validate that the "{actual_diff}" is successfully matched with "{expected_diff}" for "{type}" for "{action}" cs  service')
...
@step('I Set "{component}" api endpoint for "{action}" cs service')
def set_action_endpoint(context, component, action):
    ...
    if action == "execute" or action == "display":
        ...
        endpoint = GlobalVar.testParams.get(f'{action}EndPoint').format(serviceId,
                                                                        GlobalVar.testParams.get(f'{action}Version'))
    elif "mwr" in action:
...
@step('I Set "{component}" api request body for "{action}" cs service')
        ...
        if "e2e" in GlobalVar.testComponent[0]:
            if "execute" not in action:
                if "mwr" not in action:
                    GlobalVar.api_dict['payload'] = payloadGenerator.update_payload_message(context, GlobalVar.api_dict[
                        'payload'], GlobalVar.testParams)
                else:
                    GlobalVar.api_dict['payload'] = payloadGenerator.update_mwr_payload_message(context,
                                                                                                GlobalVar.api_dict[
                                                                                                    'payload'],
                                                                                                GlobalVar.testParams)
...
@step('I fetch "{recordType}" record for "{status}" operation from "{entity}" records for cs')
def fetch_service_status(context, recordType, status, entity):
    ...
    GlobalVar.api_dict['request_bodies'] = ApiTest.setBody(context, GlobalVar.testParams.get(f'{entity}_RequestBody'),
                                                           GlobalVar.testComponent[0].lower())

    # add wait for service record update
    time.sleep(12)

    # send request and store response
    GlobalVar.response = ApiTest.sendRequestAuth(context, GlobalVar.requestType, reqURL,
                                                 GlobalVar.api_dict['request_bodies'], username, password)
...
@step('I send request for "{action}" "{recordType}" via "{component}" for cs')
def action_on_service(context, action, recordType, component, value=''):
    ...
    if currentServiceStateFlag:
        ...
        if action == "display":
            set_action_param(context, component, action, value='')
        else:
            set_action_param(context, component, action, value='')
        ...
        GlobalVar.response = ApiTest.sendRequestjson(context, GlobalVar.requestType,
                                                     ...
                                                     GlobalVar.api_dict['request_params'],
                                                     GlobalVar.api_dict['payload'])
        print(GlobalVar.response.text)
...
@step('I validate that "{recordType}" is in expected state for "{action}" operation cs')
...
@step('I validate that service record state for "{action}" is as expected for cs')
...
@step('Validate the "{current}" and "{expected}" "{config}" in case of "{action}" for cs')
def validate_display_config(context, current, expected, config, action):
    if action in GlobalVar.scenario:
        ...
        dest_Folder = dirname(os.path.abspath(os.path.join(os.path.dirname(__file__),
                                                           "../.."))) + f"/resources/deviceConfigs/{sys.argv[1]}/reConfigs/TC_{GlobalVar.test_case}"
