...
Feature:Validate OLT birth certificate 
  ...
  # Scenario: tc_5: Onboard bng_group_id in postgres db
  #   Given I read test data for BNG controller testcases
  #   When "Insert" "bng_group_id" value in postgres db

   Scenario Outline: tc_5: This scenario validates the BNG birth certificate
   Given I read test data for BNG controller testcases
   When "Publish" "<tc_num><published>" request message to "RMQ" "polling_engine" queue for polling engine
    ...
    And Get and verify the "created" request record by "bng_group_id" from table "polling_engine_olt_birth_certificate" with "failed" state
    ...
    And Verify the "validation_result" value from "polling_engine_bng_birth_certificate"
     ...
     Examples:
     |tc_num | published                       |                                                                
     |tc_5  | _Validate_OLT_birth_certificate_failed |   

    #  Scenario: tc_5: Remove bng_group_id from postgres db
    # Given I read test data for BNG controller testcases
