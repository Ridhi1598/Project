...
Feature: This feature to Read L2 Topology OLT-BNG topology for L2 Topology with invalid request payload
  ...
  Scenario: TC33 Verify the L2 Topology Read L2 Topology with invalid request payload
    When I set CS api endpoint "/api/exchanges/%2F/{exchange_Name}/publish" for "POST" to publish the message
    When I Set Consumer POST "TC33_ReadL2TopologyInvalid.json" request Body to publish the message
    When I send CS "POST" request
    And I validate consumer expected response code "200" for "POST"
    And I set CS api endpoint "/api/queues/%2F/cs_portal_queue_test/get" for "POST"
    ...
    And I validate the consumer response body contains "cs-portal"
    And I validate the consumer response body contains "Input validation failed - Details: Input validation failed: Errors"
