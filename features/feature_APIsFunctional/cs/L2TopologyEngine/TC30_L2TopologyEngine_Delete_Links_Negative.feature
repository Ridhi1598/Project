...
Feature: This feature to Delete the links for L2 Topology with invalid request payload
  ...
  Scenario: TC30 Verify the L2 Topology delete links with invalid request payload
    When I set CS api endpoint "/api/exchanges/%2F/{exchange_Name}/publish" for "POST" to publish the message
    When I Set Consumer POST "TC30_Delete_LinksInvalid.json" request Body to publish the message
    When I send CS "POST" request
    And I validate consumer expected response code "200" for "POST"
    And I set CS api endpoint "/api/queues/%2F/cs_portal_queue_test/get" for "POST"
    ...
    And I validate the consumer response body contains "Link does not exist in L2 Topology Database - Details: Link EDTNABTFNG09-lag-87-EDTNABTFNG010-lag-67 doesn't exist in L2 Topology Database"
