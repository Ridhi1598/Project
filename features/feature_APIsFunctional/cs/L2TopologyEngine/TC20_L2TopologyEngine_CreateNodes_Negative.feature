...
Feature: This feature to Create node 1 for L2 Topology with invalid request payload
  ...
  Scenario: TC20 Verify the L2 Topology Create node 1 with invalid request payload
    When I set CS api endpoint "/api/exchanges/%2F/{exchange_Name}/publish" for "POST" to publish the message
    When I Set Consumer POST "TC20_CreateNodesInvalid.json" request Body to publish the message
    When I send CS "POST" request
    And I validate consumer expected response code "200" for "POST"
    And I set CS api endpoint "/api/queues/%2F/cs_portal_queue_test/get" for "POST"
    ...
    And I validate the consumer response body contains "Resource EDTNABTFNG09 not found in NetCracker Compass Inventory - Details: Resource EDTNABTFNG09 not found in NetCracker Compass Inventory"
