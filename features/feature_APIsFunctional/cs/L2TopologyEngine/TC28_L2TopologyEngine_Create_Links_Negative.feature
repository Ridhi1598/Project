...
Feature: This feature to Create the links for L2 Topology with invalid request payload
  ...
  Scenario: TC28 Verify the L2 Topology create links with invalid request payload
    When I set CS api endpoint "/api/exchanges/%2F/{exchange_Name}/publish" for "POST" to publish the message
    When I Set Consumer POST "TC28_Create_LinksInvalid.json" request Body to publish the message
    When I send CS "POST" request
    And I validate consumer expected response code "200" for "POST"
    And I set CS api endpoint "/api/queues/%2F/cs_portal_queue_test/get" for "POST"
    ...
    And I validate the consumer response body contains "Either source node/tp or destination node/tp does not exist in L2 engine - Details: Source termination-point EDTNABTFNG09-lag-87 does not exist in L2 DB"
