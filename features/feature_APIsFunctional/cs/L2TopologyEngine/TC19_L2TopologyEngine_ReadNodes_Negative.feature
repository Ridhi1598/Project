...
Feature: This feature to Read the nodes for L2 Topology with invalid request payload
  ...
  Scenario: TC19 Verify the Read the nodes for L2 Topology with invalid request payload
    When I set CS api endpoint "/api/exchanges/%2F/{exchange_Name}/publish" for "POST" to publish the message
    When I Set Consumer POST request Body "TC19_ReadNodesInvalid.json" for decommission
    When I send CS "POST" request
    And I validate consumer expected response code "200" for "POST"
    And I set CS api endpoint "/api/queues/%2F/cs_portal_queue_test/get" for "POST"
    ...
    And I validate the consumer response body contains "Input validation failed - Details: Input validation failed: Errors - {'query-parameters': [{0: [{'name': ['unallowed value ']}]}]}"
