  ...
  This feature validates that functionality related to commit the create L3VPN dry run request - Success
   ...
   Scenario Outline: tc_12: create the BNG dry run Success
    ...
    Examples:
     ...
     |tc_12   | _create_bng_dry_run_success  |  _create_bng_dry_run_bng_to_portal_success  |


   Scenario Outline: tc_12: commit create bng Success
    ...
    Examples:
     |tc_num  | published                            | bng_to_portal                                    |
     |tc_12   | _commit_bng_dry_run_request_success  |_commit_bng_dry_run_request_bng_to_portal_success |


   Scenario Outline: tc_12: create the L3VPN dry run Failed
    ...
    Examples:
     |tc_num   | published                     |  bng_to_portal                                |
     |tc_12    | _create_l3vpn_dry_run_failed  |  _create_l3vpn_dry_run_bng_to_portal_failed  |


    Scenario Outline: tc_12: delete bng Success
    Given I read test data for BNG controller testcases
    When "Publish" "<tc_num><published>" request message to "RMQ" "bng_controller" queue for BNG
    Then "Read" and validate that the message is consumed by orchestrator from "RMQ" "bng_controller" queue for BNG
    When Get and verify the "created" request record by "id" from table "bng_bsaf_request_tracker" with "completed" state
    ...
    Examples:
     |tc_num | published                    |  bng_to_portal                           |
     |tc_12   | _delete_bng_group_success   |  _delete_bng_group_bng_to_portal_success |
