...
Feature: Validate Ingestion engine response for create service
  This features validates Ingestion engine response for creating a service but the request times out

  Scenario: Create service with valid payload and service but request time out
    Given I set L3VPN "IngestionEngine" url
    When I set data values against "testCase_7" for Ingestion Engine
