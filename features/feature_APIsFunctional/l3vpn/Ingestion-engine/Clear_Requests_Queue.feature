@ingestionEngine @clearQueue @rmq
Feature: Clear the queue of its contents
  ...
  Scenario: Clear all the messages from the test queue
    ...
    And I set api endpoint "api/queues/%2F/tinaa-requests-tests/contents" for "DELETE" for IE
    When I send "DELETE" request for IE
    And Validate expected response code "204" for "DELETE" for IE
    Then I set api endpoint "api/queues/%2F/tinaa-requests-tests/get" for "POST" for IE
    And I Set POST request Body "rmq_get_message.json" for "tinaa-requests-tests" for IE
