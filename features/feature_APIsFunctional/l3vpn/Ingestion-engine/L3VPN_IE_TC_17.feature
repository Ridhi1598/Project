...
Feature: Validate Ingestion engine response for delete service
  This features validates Ingestion engine response for deleting a service in terminated state

  Scenario: Delete a service which is in terminated state
    Given I set L3VPN "IngestionEngine" url
    And I set data values against "testCase_17" for Ingestion Engine
