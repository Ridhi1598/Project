@ingestionEngine
Feature: Validate Ingestion engine response for Activate inactive service
  This features validates Ingestion engine response for Ac inactive service

  Scenario: Add an existing service with valid payload and get success
	Given I set L3VPN "IngestionEngine" url
    When I set data values against "testCase_27" for Ingestion Engine
