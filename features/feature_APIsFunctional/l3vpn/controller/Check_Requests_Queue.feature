...
Feature: Clear the queue of its contents
  ...
  Scenario: Check if callback queue exists
    ...
    And I validate "tinaa-requests-tests" queue is binded to "l3vpn-ingestion" exchange
    ...
    Given I set api endpoint "api/bindings/%2F/e/l3vpn-ingestion/q/tinaa-requests-tests" for "POST"
    And I Set POST request Body "bindQueue.json" for "tinaa-l3vpn-requests"
    When I send "POST" request
    And I validate expected response code "201" for "POST"
    And Validate response header "location" for "tinaa-requests-tests/tinaa-l3vpn-requests"
    ...
    And Validate that "tinaa-requests-tests" queue "has" "l3vpn-ingestion" exchange as source