@orchestrator @tc122 @outOfScope
Feature: Validate orchestrator response for modify HS service request
  This features validates orchestrator response for failed create request since request timed out in resource validation

  Scenario: Create Service request failure since request timed out in resource validation
    ...
    But Validate that a "no" message is published to controller
