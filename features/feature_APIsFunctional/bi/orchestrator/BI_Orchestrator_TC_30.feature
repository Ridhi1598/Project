@orchestrator @tc30 @rollbackService
Feature: Validate orchestrator response for executing rollback request for create mwr service
  This features validates orchestrator response for executing rollback request for create mwr service
  ...
  Scenario: Update ES record for a create base service request record in "bi-orchestrator-requests" index
    Given I set BI "ES" url
    Given I create and assign document id for record creation
  ...
  Scenario: Execute rollback request for create mwr service with Failed Response
    Given I set data values against testcase
