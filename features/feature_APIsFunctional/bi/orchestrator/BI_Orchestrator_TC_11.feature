@orchestrator @tc11 @updateService
Feature: Validate orchestrator response for successful port activation request
  This features validates orchestrator response for successful port activation request
  ...
  Scenario: Successful port activation request
    Given I set data values against testcase
    ...
    And Validate the value of "admin-state" is "up"
    And I set BI "RMQ" url
