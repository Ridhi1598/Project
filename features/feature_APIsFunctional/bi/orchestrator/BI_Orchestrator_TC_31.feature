@orchestrator @tc31 @rollbackService
Feature: Validate orchestrator response for executing rollback request for delete mwr service
  This features validates orchestrator response for executing rollback request for delete mwr service
  ...
  Scenario: Update ES record for a create request record in "bi-orchestrator-requests" index
    Given I set BI "ES" url
    Given I create and assign document id for record creation
    And I set api endpoint for "create" a "requests" record for "deleteMwrService"
    And I set api request body for "create" a "requests" record for "deleteMwrService"
    When I send HTTP request for "create" service
    Then I validate that the "requests" record is "created" in "bi-orchestrator-requests" index for "deleteMwrService"

  Scenario: Execute rollback request for delete mwr service with Success Response
    Given I set data values against testcase
  ...
  Scenario: Delete ES record for created request in "bi-orchestrator-requests" index
    Given I set BI "ES" url
    And I set api endpoint for "delete" a "requests" record for "deleteMwrService"
    And I set api request body for "delete" a "requests" record for "deleteMwrService"
    When I send HTTP request for "delete" service
    Then I validate that the "requests" record is "deleted" in "bi-orchestrator-requests" index for "deleteMwrService"