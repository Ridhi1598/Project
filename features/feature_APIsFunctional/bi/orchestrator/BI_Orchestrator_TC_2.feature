@orchestrator @tc2 @createService
Feature: Validate orchestrator response for create service request
  This features validates orchestrator response for failed create service request due to error from NSO

  Scenario: Create Service request failure due to error from NSO
    ...
    And Validate there are no unicode characters in the response