@orchestrator @tc148 @displayConfig
Feature: Validate orchestrator response for successful display config request
  ...
  Scenario: Successful display config request for Modify
    Given I set data values against testcase
    ...
    And Validate the "audit" value in the response for "expected" config
    And I set BI "RMQ" url
    ...
    And I validate the "success" response and format for "expected" config
