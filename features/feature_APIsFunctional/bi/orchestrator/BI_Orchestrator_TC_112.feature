@orchestrator @tc112 @updateService
Feature: Validate orchestrator response for modify HS service request
  This features validates orchestrator response for failed modify service request due to non existent port
  ...
  Scenario: Modify service by changing port but fails due to non existent port: valid port but not found on RE
    Given I set data values against testcase
