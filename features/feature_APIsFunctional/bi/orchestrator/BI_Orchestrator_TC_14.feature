@orchestrator @tc14 @mwr
Feature: Validate orchestrator response for successful create MWR request
  This features validates orchestrator response for successful create MWR request
  ...
  Scenario: Successful create MWR request
    Given I set data values against testcase
    ...
    And Validate there is "2" site instance under site
    And Validate the "base" service with "site-network-access-id" has "available" value "1"
    And Validate the "MWR" service with "site-network-access-id" has "available" value "10"
    And I set BI "RMQ" url
