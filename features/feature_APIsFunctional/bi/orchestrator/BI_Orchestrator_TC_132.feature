@orchestrator @tc132 @createService
Feature: Validate orchestrator response for create GPON service request
  This features validates orchestrator response for failed create GPON Service which passes Vlan validation but fails due to error from mediation

  Scenario: Create GPON Service which passes Vlan validation but fails due to error from mediation
