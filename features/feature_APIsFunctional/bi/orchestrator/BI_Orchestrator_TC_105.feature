@orchestrator @tc105 @createService
Feature: Validate orchestrator response for create HS service request
  This features validates orchestrator response for failed create service request due to non existent port

  Scenario: Create Service request failure due to non existent port: valid port format but not found on device
