@orchestrator @tc127 @createService
Feature: Validate orchestrator response for create DSL service request
  This features validates orchestrator response for failed create service request since vlan id is not available

  Scenario: Create Service request failure due to vlan id not available
    ...
    And I validate that a "request" record is "not created" in "bi-orchestrator-requests" index
