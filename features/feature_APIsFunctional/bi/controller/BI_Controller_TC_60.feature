@controller
Feature: Validate controller response success message for display before and after config for an update request
...
@displayConfig
  Scenario: Display before and after config for an update request
    ...
    And I set data values against test case "60"
    And I Set headers "Content" and "Authorization"
    And I Set api endpoint and request Body
    And I Set query parameters for controller request for "before"
    When I Send HTTP request for controller
