...
Feature: Validate controller response for create service
  This features validates controller response for creating a failed GPON service request due to unavailable vlan Id

  Scenario: Create service with valid payload and service but request fails
    Given I set BI "controller" url
    And I set data values against test case "141"
