@controller
Feature: Validate controller response for updating a request in pending state
  This features validates controller response for updating a request in pending state

@updateFromServiceQueue
  Scenario: Updating a pending service request
    Given I set BI "controller" url
    And I set data values against test case "55"
    ...
    And I validate response body should have "status" as "error"
