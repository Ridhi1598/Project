...
Feature: Validate controller response for delete service
  This features validates controller response for removing MWR service but request fails

  Scenario: Remove an MWR service from an existing service but the request fails
    Given I set BI "controller" url
    And I set data values against test case "28"
