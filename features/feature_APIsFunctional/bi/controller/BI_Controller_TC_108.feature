...
Feature: Validate controller response in case of modify service rollback success
  ...
  Scenario: Modify service rollback success: Success
    ...
    And I set data values against test case "108"
    ...
    And Mock "rollback-mwr-success" response to be published to RMQ "tinaa-bi-request-callbacks" queue
    And I validate that a "service" record is found in "bi-controller-services" index
    ...
    And Validate that service "mwr_id" is "not None"
    ...
    And Validate that a callback is sent to PubSub for "successRollbackMwr"
    ...
    And Validate that the callback info has "@rollbackBaseService" set as "No"
    And Validate that the callback info has "Value" as "serviceId"
    And Validate that "@rollbackMWRService" field is "present" in callback info
    And Validate that the callback info has "@rollbackMWRService" set as "Yes"
    And Validate that the callback info has "Value" as "mwrId"