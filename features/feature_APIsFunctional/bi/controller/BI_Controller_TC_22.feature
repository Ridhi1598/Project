...
Feature: Validate controller response for update service
  This features validates controller response for a failed activation request

  Scenario: Activate an existing service but the request fails
    Given I set BI "controller" url
    And I set data values against test case "22"
