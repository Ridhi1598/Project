...
Feature: Validate controller response for activate port
  This features validates controller response for port activation for service in progress state

  Scenario: Activate port for a service in progress state
    Given I set BI "controller" url
    And I set data values against test case "13"
