...
Feature: Validate controller response for update service
  This features validates controller response for a timed out activation request

  Scenario: Activate an existing service but the request times out
    Given I set BI "controller" url
    And I set data values against test case "23"
