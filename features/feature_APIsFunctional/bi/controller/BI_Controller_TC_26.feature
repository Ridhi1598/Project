...
Feature: Validate controller response for update service
  This features validates controller response for adding am MWR service but request times out

  Scenario: Add MWR to an existing service but the request times out
    Given I set BI "controller" url
    And I set data values against test case "26"
