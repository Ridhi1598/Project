Feature: Validate evpn response for delete-audit service request
  This features validates evpn response for timed out delete-audit service request
  ...
  Scenario: Delete Service request with no response
    ...
    Then "Read" and validate that a "delete-audit" "timeout_callback" response is published in the "RMQ" "orchestrator-callback" queue
