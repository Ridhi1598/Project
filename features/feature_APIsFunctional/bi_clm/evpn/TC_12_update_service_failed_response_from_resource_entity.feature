Feature: Validate evpn response for create service request
  ...
  Scenario: Create Service request with failure response from resource entity
    ...
    Then "Read" and validate that a "update" "failed_callback" response is published in the "RMQ" "orchestrator-callback" queue
    ...
    Then Validate that the "before" and "after" values for "bandwidth" are "not updated"