Feature: Validate l3vpn controller's delete operation
  This features validates a failed delete request of vpn-service

  Scenario: Delete a VPN having only one node. Controller will trigger the ep without node_id param
    ...
    Then Sending delete with audit true payload to L3VPN via RMQ
