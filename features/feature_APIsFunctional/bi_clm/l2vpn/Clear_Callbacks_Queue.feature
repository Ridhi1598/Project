...
Feature: Clear the queue of its contents
  ...
  Scenario: Clear all the messages from the test queue
    Given I set "RMQ" url
    And I set api endpoint "/api/queues/%2F/tinaa-orch-callbacks-tests/contents" for "DELETE"
    When I send "DELETE" request
    And I validate expected response code "204" for "DELETE"
    Then I set api endpoint "/api/queues/%2F/tinaa-orch-callbacks-tests/get" for "POST"
    And I Set POST request Body "rmq_get_message.json" for "tinaa-orch-callbacks-tests"
