...
Feature: Clear the queue of its contents
  ...
  Scenario: Check if callback queue exists
    Given I set "RMQ" url
    And I set api endpoint "/api/queues/%2F/tinaa-orch-callbacks-tests" for "GET"
    When I send "GET" request
    Then I extract the response code value
    And I validate "tinaa-orch-callbacks-tests" queue is binded to "com.telus.tinaa.bsaf.clm.bi" exchange
    Given I validate bind status from the previous step
    And I set api endpoint "/api/queues/%2F/tinaa-orch-callbacks-tests" for "PUT"
    And I Set PUT request Body "rmq_create_queue.json" for "tinaa-orch-callbacks-tests"
    Then I send "PUT" request
    When I validate expected response code "201" or "204" for "PUT"
    Given I set api endpoint "/api/bindings/%2F/e/com.telus.tinaa.bsaf.clm.bi/q/tinaa-orch-callbacks-tests" for "POST"
    ...
    And Validate response header "location" for "tinaa-orch-callbacks-tests/com.telus.tinaa.bsaf.clm.bi.orchestrator.response"
    ...
    And Validate that "tinaa-orch-callbacks-tests" queue "has" "com.telus.tinaa.bsaf.clm.bi" exchange as source