...
Feature: Validate ingestion response in case of modify service rollback success
  ...
  Scenario: Modify service rollback success: Success
    ...
    And Mock "display-rollback-config-success" response to "RMQ" "com.telus.tinaa.bsaf.clm.bi.ingestion.response" queue
    ...
    And Mock "rollback-success" response to "RMQ" "com.telus.tinaa.bsaf.clm.bi.ingestion.response" queue
    And I validate that a "service" record is found in "clm-ingestion-service-services" index
    And I validate record format for "service" record
