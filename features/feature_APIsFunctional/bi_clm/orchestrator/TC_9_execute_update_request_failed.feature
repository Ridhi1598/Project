...
Feature: Validate orchestrator response for update service request
  ...
  Scenario: Update Service request with failure response from l3vpn controller for primary node
    ...
    Then "Read" and validate that a "update" "failed_l3vpn_callback" message is published in the "RMQ" "orchestrator-callback" queue
