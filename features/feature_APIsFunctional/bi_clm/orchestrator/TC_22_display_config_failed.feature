...
Feature: Validate orchestrator response for display config request
  ...
  Scenario: Display config request with failed response from l3vpn controller for expected secondary l3vpn node
    ...
    When "Publish" "l3vpn-display-expected-failed-callback" response to "RMQ" "l3vpn-callback" queue for "secondary_l3vpn_node"
