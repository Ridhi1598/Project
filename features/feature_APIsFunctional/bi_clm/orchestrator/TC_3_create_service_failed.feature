...
Feature: Validate orchestrator response for create service request
  ...
  Scenario: Create Service request with failure response from l3vpn controller for secondary node
    ...
    When "Publish" "l3vpn-create-failed-callback" response to "RMQ" "l3vpn-callback" queue for "secondary_l3vpn_node"
