...
Feature: Validate orchestrator response for display config request
  ...
  Scenario: Display config request with failed response from l3vpn controller for current primary l3vpn node
    ...
    Then "Read" and validate that a "display" "secondary_l3vpn_node_current" message is published in the "RMQ" "l3vpn-publish" queue
    When "Publish" "l3vpn-display-current-in-process-callback" response to "RMQ" "l3vpn-callback" queue for "secondary_l3vpn_node_current"
    When "Publish" "l3vpn-display-current-success-callback" response to "RMQ" "l3vpn-callback" queue for "secondary_l3vpn_node_current"
    ...
    When "Publish" "l3vpn-display-current-failed-callback" response to "RMQ" "l3vpn-callback" queue for "primary_l3vpn_node"
