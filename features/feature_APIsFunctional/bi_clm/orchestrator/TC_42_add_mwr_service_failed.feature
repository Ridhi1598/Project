...
Feature: Validate orchestrator response for add mwr service request
  This features validates orchestrator response for failed add mwr service request
  ...
  Scenario: Add Mwr Service request with failed response for primary node
    ...
    When "Publish" "l3vpn-update-failed-callback" response to "RMQ" "l3vpn-callback" queue for "primary_l3vpn_node"
    Then I "read" and validate that a "csid" record is "created" in orchestrator "ES" records
