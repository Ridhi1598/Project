...
Feature: Validate orchestrator response for rate limit request
  This features validates orchestrator response for rejected rollback service request
  ...
  Scenario: Rollback Service request which is rejected by orchestrator
    ...
    Then "Read" and validate that a "rollback" "failed_evpn_callback" message is published in the "RMQ" "orchestrator-callback" queue
    Then I "read" and validate that a "service" record is "created" in orchestrator "ES" records
