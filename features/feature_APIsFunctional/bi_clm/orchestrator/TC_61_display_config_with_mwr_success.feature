@display @TC61
Feature: Validate orchestrator response for display service(with mwr) request
  This features validates orchestrator response for successful display service(with mwr) request
  ...
  Scenario: Display config request with success response
    When "Publish" "display_base" service message to "RMQ" "orchestrator-publish" queue
    ...
    Then "Read" and validate that a "display" "mwr_l3vpn_node" message is published in the "RMQ" "l3vpn-publish" queue
    When "Publish" "l3vpn-display-current-in-process-callback" response to "RMQ" "l3vpn-callback" queue for "mwr_l3vpn_node"
    When "Publish" "l3vpn-display-current-success-callback" response to "RMQ" "l3vpn-callback" queue for "mwr_l3vpn_node"
    ...
    Then "Read" and validate that a "display" "mwr_l3vpn_node" message is published in the "RMQ" "l3vpn-publish" queue
    When "Publish" "l3vpn-display-expected-in-process-callback" response to "RMQ" "l3vpn-callback" queue for "mwr_l3vpn_node"
    When "Publish" "l3vpn-display-expected-success-callback" response to "RMQ" "l3vpn-callback" queue for "mwr_l3vpn_node"
    Then "Read" and validate that a "display" "secondary_evpn_profile" message is published in the "RMQ" "evpn-publish" queue
    When "Publish" "evpn-display-success-callback" response to "RMQ" "evpn-callback" queue for "secondary_evpn_profile"
    Then "Read" and validate that a "display" "primary_evpn_profile" message is published in the "RMQ" "evpn-publish" queue
    When "Publish" "evpn-display-success-callback" response to "RMQ" "evpn-callback" queue for "primary_evpn_profile"
    ...
    When "Publish" "l3vpn-display-expected-success-callback" response to "RMQ" "l3vpn-callback" queue for "primary_l3vpn_node"
    Then "Read" and validate that a "display" "get-config-success_callback" message is published in the "RMQ" "orchestrator-callback" queue
    Then I "read" and validate that a "csid" record is "created" in orchestrator "ES" records
