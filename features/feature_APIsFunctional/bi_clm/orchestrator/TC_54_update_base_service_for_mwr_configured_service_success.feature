...
Feature: Validate orchestrator response for update service(with mwr) request
  This features validates orchestrator response for successful update service(with mwr) request
  ...
  Scenario: Update Service(with mwr) request with success response
    ...
    Then "Read" and validate that a "update" "mwr_l3vpn_node" message is published in the "RMQ" "l3vpn-publish" queue
    When "Publish" "l3vpn-in-process-callback" response to "RMQ" "l3vpn-callback" queue for "mwr_l3vpn_node"
    When "Publish" "l3vpn-update-success-callback" response to "RMQ" "l3vpn-callback" queue for "mwr_l3vpn_node"
    Then I "read" and validate that a "csid" record is "created" in orchestrator "ES" records
    ...
    Then Validate that the "before" and "after" values for "prefix" is "updated"
    Then Validate that the "before" and "after" values for "speed" is "updated"
    Then "Read" and validate that a "update" "success_callback" message is published in the "RMQ" "orchestrator-callback" queue
