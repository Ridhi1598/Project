...
Feature: Validate orchestrator response for delete service(with mwr) request
  ...
  Scenario: Delete Service(with mwr) request with failed response for mwr node
    ...
    When "Publish" "l3vpn-delete-failed-callback" response to "RMQ" "l3vpn-callback" queue for "mwr_l3vpn_node"
    Then I "read" and validate that a "csid" record is "created" in orchestrator "ES" records
