...
Feature: Validate orchestrator response for update service request
  ...
  Scenario: Update Service request with failure response from evpn controller for secondary evpn profile
    ...
    When "Publish" "evpn-update-failed-callback" response to "RMQ" "evpn-callback" queue for "secondary_evpn_profile"
    Then I "read" and validate that a "csid" record is "created" in orchestrator "ES" records
