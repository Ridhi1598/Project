...
Feature: Validate orchestrator response for update service(with mwr) request
  ...
  Scenario: Update Service(with mwr) request with failed response for mwr node
    ...
    When "Publish" "l3vpn-update-failed-callback" response to "RMQ" "l3vpn-callback" queue for "mwr_l3vpn_node"
    Then I "read" and validate that a "service" record is "created" in orchestrator "ES" records
