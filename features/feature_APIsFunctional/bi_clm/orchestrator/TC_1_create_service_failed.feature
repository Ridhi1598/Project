...
Feature: Validate orchestrator response for create service request
  ...
  Scenario: Create Service request with failure response from evpn controller for secondary evpn profile
    ...
    When "Publish" "evpn-create-failed-callback" response to "RMQ" "evpn-callback" queue for "secondary_evpn_profile"
    Then I "read" and validate that a "csid" record is "created" in orchestrator "ES" records
