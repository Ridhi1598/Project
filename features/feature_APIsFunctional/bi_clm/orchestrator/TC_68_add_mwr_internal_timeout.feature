@internalTimeout @TC68
Feature: Validate orchestrator response for add mwr request
  This features validates orchestrator response for add mwr request which timed out
  ...
  Scenario: Add Mwr Service request with no response for mwr node
    ...
    Then I validate that the "service" record has "2" "site-network-access" records for "mwr_service"
    ...
    Then "Read" and validate that a "create" "failed_callback" message is published in the "RMQ" "orchestrator-callback" queue
    ...
    Then I "read" and validate that a "csid" record is "created" in orchestrator "ES" records
    And Validate that a "mwr_l3vpn_node" is "removed from" the "csid" record
    And I validate that "csid" record "in_progress" value is "false"
    When I "read" and validate that a "service" record is "created" in orchestrator "ES" records
    Then I validate that the "service" record has "2" "site-network-access" records for "mwr_service"
    Then I "read" and validate that a "request" record is "created" in orchestrator "ES" records
    And I validate that "request" record "state" value is "timeout"
