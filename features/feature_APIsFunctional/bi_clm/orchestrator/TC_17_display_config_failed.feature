...
Feature: Validate orchestrator response for display config request
  ...
  Scenario: Display config request with failed response from evpn controller for current evpn primary profile
    ...
    When "Publish" "evpn-display-failed-callback" response to "RMQ" "evpn-callback" queue for "primary_evpn_profile_current"
    Then I "read" and validate that a "csid" record is "created" in orchestrator "ES" records
