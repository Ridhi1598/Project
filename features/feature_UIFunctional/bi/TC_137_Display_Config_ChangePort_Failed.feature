@portal @tc137
Feature: Resource Validation for change port value scenario

  Scenario: Change Port : Display Config : Fails due to unavailable port
    Given I set data values against testcase "137"
    ...
    Then Mock "display-expected-config-port-failed" response to be published to RMQ "tinaa-bi-request-callbacks" queue
