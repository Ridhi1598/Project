@OA2501-1
Feature: TC_43: Create and Delete GRF QPV test
...
Scenario: Create/Update service with valid payload and in valid state
  ...
  When I run "show running-config policy-map WAN_OUT_Global" command and store the device config
  Then I validate the output of "policy-map_WAN_OUT_Global" section at "default-level"
...
@deleteService @TC_103
Scenario: Delete the created service for TC_103 which is in valid state
  When I set L3VPN "REST" url
  Then I set api endpoint for testcase "103" for l3vpn
